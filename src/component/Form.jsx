        ...
        <div className='container'>
            ...
            <form action="" onSubmit={handleSubmit}>
                <div className="form">
                    ...
                    <input type="text" maxLength={50} selected={title}
                    onChange={(event) => {
                        setTitle(event.target.value);
                    }} 
                    // onChange={(event) =>
                    // (event.target.value.length > 5) ? event.target.readOnly = true : event.target.readOnly=false} 
                    name="title" className="title" placeholder="title"/>
                    ...
                    <div className="drop-down">
                        <select className="" onChange={(event) => {
                            ...
                            {assignment.map(
                                (assignment, index) => <option key={index} >{assignment}</option>
                            )}
                        </select>
                        <select className="" onChange={(event) => {
                            ...
                            <option>Level</option>
                            {level.map(
                                (level, index) => <option key={index} >{level}</option>
                            )}
                        </select>
                        <select className="" onChange={(event) => {
                    ...
                    placeholder={RCA}
                    onClick={(event) => {
                        stus==="Fixed" ? event.target.readOnly = false : event.target.readOnly = true
                        setRCA(event.target.value)
                    }} 
                    className="" name="" id="" cols="30" rows="10"></textarea>
